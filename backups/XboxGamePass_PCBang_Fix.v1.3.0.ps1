...
function Clear-StoreCache {
    Write-Step '[7/8] Microsoft Store 캐시 정리 중...'
...
# ----------------------------------------------------------
Write-Step '[1/8] Windows Update 차단 정책 제거 중...'
...
# ----------------------------------------------------------
Write-Step '[2/8] 서비스 잠금(WubLock/ACL/svchost) 복구 중...'
...
# ----------------------------------------------------------
Write-Step '[3/8] Windows Update 작업 폴더 복구 중...'
...
# ----------------------------------------------------------
Write-Step '[4/8] Windows Update 관련 서비스 활성화 중...'
$services = @(
...
Write-Step '[5/8] Windows Update 검색 API 검증 중...'
$script:WuSearchOk = Test-WindowsUpdateSearch
Write-Log ''

# ----------------------------------------------------------
# 6. Windows Update에서 최신 루트 인증서 묶음 다운로드
# ----------------------------------------------------------
Write-Step '[6/8] 최신 루트 인증서 묶음 생성 중 (수십 초 소요될 수 있음)...'
...
# ----------------------------------------------------------
# 7. Microsoft Store 캐시 정리
# ----------------------------------------------------------
Clear-StoreCache
Write-Log ''

# ----------------------------------------------------------
# 8. 신뢰할 수 있는 루트 인증 기관 저장소에 일괄 임포트
# ----------------------------------------------------------
Write-Step '[8/8] 신뢰할 수 있는 루트 인증 기관에 임포트 중...'
if ($NoCertImport) {
...
# ----------------------------------------------------------
Write-Log '====================================================' ([ConsoleColor]::Cyan)
if ($script:RootImportOk -and $script:ServicesOk -and $script:WuSearchOk) {
    ...
    Write-Log '       Xbox 앱 제품 ID: 9MV0B5HZVK9Z' ([ConsoleColor]::Green)
    Open-XboxStorePage
...
if ($script:RootImportOk -and $script:WuSearchOk) {
    Wait-BeforeExit
    exit 0
}
