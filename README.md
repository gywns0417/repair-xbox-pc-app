...
| `wuauserv`, `DoSvc`가 Disabled에서 풀리지 않음 | WubLock, 서비스 레지스트리 ACL, svchost 그룹 복구 |
| Store 설치가 `0x8024500c`로 실패 | Windows Update 정책 캐시(`UpdatePolicy\GPCache`)와 Store 캐시 정리 |
| Xbox 앱 페이지에서 Store 클라이언트 업데이트가 필요하다고 표시 | `WSReset.exe -i`로 Microsoft Store 클라이언트 업데이트 |
| Xbox 앱 실행 시 `0x80073cf3` 발생 | Gaming Services와 `Microsoft.NET.Native.Framework/Runtime 2.2` 의존성 복구 |
...
| 2 | WSUS/인터넷 Windows Update 차단 값 제거: `DoNotConnectToWindowsUpdateInternetLocations`, `WUServer`, `WUStatusServer`, `UpdateServiceUrlAlternate` |
| 3 | WubLock 또는 PC방 관리툴이 잠근 서비스 레지스트리 ACL 복구 |
| 4 | `svchost` 그룹에서 빠진 `wuauserv`/`DoSvc` 항목 복구 |
| 5 | Windows Update 내부 정책 캐시(`UpdatePolicy\GPCache`)에서 남은 차단 값 제거 |
| 6 | Windows Update 작업 폴더 복구: `SoftwareDistribution`, `DataStore`, `Download`, `catroot2` |
| 7 | `wuauserv`, `BITS`, `CryptSvc`, `UsoSvc`, `DoSvc`, `InstallService`, `WaaSMedicSvc` 활성화 및 시작 |
| 8 | Windows Update 검색 API가 `0x8024500c` 없이 동작하는지 검증 |
| 9 | 오래된 Microsoft Store 클라이언트를 `WSReset.exe -i`로 업데이트 |
| 10 | Microsoft Store 캐시 정리 |
| 11 | `certutil -generateSSTFromWU`로 루트 인증서 저장소 파일 생성 후 `certutil -addstore -f Root`로 임포트 |
| 12 | Store 설치 API로 `Gaming Services(9MWPM2CQNLHN)` 설치/복구 |
| 13 | `Microsoft.NET.Native.Framework.2.2` 및 `Microsoft.NET.Native.Runtime.2.2` 요구 버전 검증 |
| 14 | `GamingServices`, `GamingServicesNet` 서비스 시작 |
| 15 | 복구 성공 시 Xbox 앱 실행 또는 Xbox 앱 Store 페이지 열기 |
